...
import { C, fmt, DEFAULT_HFILT } from "../../utils/constants";
import { Screen, HistorialFilter } from "../../models/types";
import { AppHeader, HistorialFilterModal, VerDetallesBtn } from "../../components/Shared";
import { useSales } from "../../context/SalesContext";
import { useInventory } from "../../context/InventoryContext";

export function HistorialScreen({ goTo }:{ goTo:(s:Screen)=>void }) {
  const { orders } = useSales();
  const { products } = useInventory();

  const [search,setSearch]=useState(""); 
  const [expanded,setExpanded]=useState<string|null>(null);
  const [hFilter,setHFilter]=useState<HistorialFilter>(DEFAULT_HFILT); 
  const [showHFilter,setShowHFilter]=useState(false);
    ...
    <div className="flex flex-col h-full" style={{ background:C.bg }}>
      ...
      <div className="flex-1 overflow-y-auto px-5 pt-5 pb-10">
        <div className="flex flex-col gap-5">
          ...
          {Array.from(new Set(filtered.map(s=>s.date||"hoy"))).map(day=>{ 
            const daySales=filtered.filter(s=>(s.date||"hoy")===day); 
            if(daySales.length===0) return null; 
            return (
            <div key={day}>
              <p className="text-xs font-semibold tracking-widest uppercase mb-3" style={{ color:C.txt2 }}>{day}</p>
