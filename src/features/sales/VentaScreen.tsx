...
import { ArrowLeft, Bell, Search, Filter, Scale, Check, X, ClipboardList } from "lucide-react";
import { C, fmt, DEFAULT_FILTER } from "../../utils/constants";
import { Screen, Product, Category, FilterState, CartItem } from "../../models/types";
import { FilterModal } from "../../components/Shared";
...
export function VentaScreen({ goTo, onBell, notifCount, onSaleConfirmed }:{
  goTo:(s:Screen)=>void; onBell:()=>void; notifCount:number; onSaleConfirmed:(data:{total:number,method:string,cart:CartItem[],id:string})=>void;
}) {
  const { products } = useInventory();
  const { cart, setCart, confirmSale } = useSales();
  ...
  const [payModalOpen, setPayModalOpen] = useState<"efectivo"|"tarjeta"|null>(null);
  ...
  const handleConfirm = async () => {
    if (payModalOpen) {
      await confirmSale(payModalOpen, (total, method, cartCopy, id) => {
        setPayModalOpen(null);
        onSaleConfirmed({ total, method, cart: cartCopy, id });
      });
    }
  };
  ...
  return (
    <div className="flex flex-col h-full relative" style={{ background:C.bg }}>
      ...
      <div className="shrink-0 w-full px-3 md:px-0 md:max-w-6xl md:mx-auto pb-4 md:pb-6 pt-2" style={{ background:C.bg }}>
        <div className="w-full rounded-2xl shadow-2xl overflow-hidden border p-4" style={{ background:"#fff", borderColor:C.border }}>
          ...
          <div className="flex gap-2">
            ...
            <button onClick={()=>subtotal>0&&setPayModalOpen(payMode)} disabled={subtotal===0} className="flex-[1.5] h-11 rounded-full text-sm font-semibold text-white disabled:opacity-40 active:scale-95 transition-transform ml-1" style={{ background:C.green }}>Finalizar →</button>
          </div>
        </div>
      </div>
      ...
      {payModalOpen==="efectivo" && <PagoEfectivoModal cart={cart} onConfirm={handleConfirm} onCancel={()=>setPayModalOpen(null)} />}
      {payModalOpen==="tarjeta" && <PagoTarjetaModal cart={cart} onConfirm={handleConfirm} onCancel={()=>setPayModalOpen(null)} />}
    </div>
  );
}
